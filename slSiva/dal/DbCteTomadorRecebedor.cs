...
namespace DAL
{
    public class DbCteTomadorRecebedor : IDisposable
    {
                                            ...
                                            DateTime? pDtInicioIntervalo, DateTime? pDtFinalIntervalo, decimal? pCdMunicipio)
        {
            using (OracleConnection cnn = new OracleConnection(Properties.Settings.Default.ConnectionString))
            {
                ...
                return cnn.Query<CteTomadorRecebedor>("ADM_OBJETOS.CTEPKG_CONHECIMENTO_TRANSPORTE.SP_CONSULTA_CTE_TOM_RECEB", param: parameters, commandType: CommandType.StoredProcedure);                
            }
        }     
